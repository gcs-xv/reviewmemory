...
def build_block_with_meta(no, row, visit_key, base_date):
    ...
    Logika baru:
    - Kunjungan 1:
        • Diagnosa: kosong (sesuai contoh)
        • Tindakan: [Konsultasi, Periapikal X-ray gigi {g} / OPG X-Ray]
        • Kontrol: Pro <Ekstraksi|Odontektomi> gigi {g} dalam LA (H+7 dari HARI INI)
    - Kunjungan 2:
        • Jika impaksi (..8): Diagnosa=Impaksi gigi {g}..., Tindakan=Odontektomi gigi {g}
        • Jika non-impaksi:   Diagnosa=Gangren pulpa gigi {g} / Gangren radiks gigi {g}, Tindakan=Ekstraksi gigi {g}
        • Kontrol: POD IV (pakai compute_kontrol_text(base PERIODE))
        • Opsi "penyulit/open methode" dihapus
    - Kunjungan 3/4/5:
        • Diagnosa = POD {III|VII|XIV} <Ekstraksi|Odontektomi> gigi {g}
        • Tindakan sesuai template (cuci luka/rujuk)
        • Kontrol tetap via compute_kontrol_text (kecuali '-' dipakai langsung)
    """
    tpl_key = normalize_visit(visit_key or row.get("visit") or "(Pilih)")

    g_raw = (row.get("gigi") or "").strip()
    g_clean = re.sub(r"\D", "", g_raw)
    tooth_label = g_clean if g_clean else "xx"
    is_imp = is_impaksi_tooth(g_clean)

    def op_word():
        return "Odontektomi" if is_imp else "Ekstraksi"
    ...
    if tpl_key == "Kunjungan 1":
        diagnosa_txt = ""  # sesuai contohmu
        tindakan_list = [
            "Konsultasi",
            f"Periapikal X-ray gigi {tooth_label} / OPG X-Ray",
        ]
        # Kontrol: H+7 dari tanggal hari-ini (bukan PERIODE)
        # NOTE: jika ingin pakai zona Asia/Jakarta secara eksplisit:
        # from zoneinfo import ZoneInfo
        # today_local = datetime.now(ZoneInfo("Asia/Jakarta")).date()
        # else fallback ke date.today()
        hplus = (date.today() + timedelta(days=7)).strftime("%d/%m/%Y")
        kontrol_txt = f"Pro {op_word().lower()} gigi {tooth_label} dalam lokal anestesi ({hplus})"

    # ===== Kunjungan 2 =====
    elif tpl_key == "Kunjungan 2":
        if is_imp:
            diagnosa_txt = f"Impaksi gigi {tooth_label} kelas xx posisi xx Mesioangular"
            tindakan_list = [f"Odontektomi gigi {tooth_label} dalam lokal anestesi"]
        else:
            diagnosa_txt = f"Gangren pulpa gigi {tooth_label} / Gangren radiks gigi {tooth_label}"
            tindakan_list = [f"Ekstraksi gigi {tooth_label} dalam lokal anestesi"]
        kontrol_tpl = "POD IV (xx/04/2025)"
        kontrol_txt = compute_kontrol_text(kontrol_tpl, diagnosa_txt, base_date)

    # ===== Kunjungan 3 =====
    elif tpl_key == "Kunjungan 3":
        diagnosa_txt = f"POD III {op_word()} gigi {tooth_label} dalam lokal anestesi"
        tindakan_list = ["Cuci luka intraoral dengan NaCl 0,9%"]
        kontrol_tpl = "POD VII (xx/04/2025)"
        kontrol_txt = compute_kontrol_text(kontrol_tpl, diagnosa_txt, base_date)

    # ===== Kunjungan 4 =====
    elif tpl_key == "Kunjungan 4":
        diagnosa_txt = f"POD VII {op_word()} gigi {tooth_label} dalam lokal anestesi"
        tindakan_list = ["Cuci luka intra oral dengan NaCl 0,9%", "Aff hecting"]
        kontrol_tpl = "POD XIV (xx/04/2025)"
        kontrol_txt = compute_kontrol_text(kontrol_tpl, diagnosa_txt, base_date)

    # ===== Kunjungan 5 =====
    elif tpl_key == "Kunjungan 5":
        diagnosa_txt = f"POD XIV {op_word()} gigi {tooth_label} dalam lokal anestesi"
        tindakan_list = ["Kontrol luka post operasi", "Rujuk balik FKTP"]
        kontrol_txt = "-"

    else:
        # fallback: pakai template lama lalu sesuaikan gigi + filter impaksi
        tpl = VISIT_TEMPLATES.get(tpl_key, VISIT_TEMPLATES["(Pilih)"])
        diagnosa = replace_gigi(tpl["diagnosa"], tooth_label)
        tlist = [replace_gigi(t, tooth_label) for t in tpl["tindakan"]]
        kontrol = replace_gigi(tpl["kontrol"], tooth_label)
        diagnosa, tlist, kontrol = filter_for_tooth(diagnosa, tlist, kontrol, tooth_label)
