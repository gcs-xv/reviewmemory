...
def load_review_map(supabase, periode_date: str | None, file_name: str | None):
    if not periode_date:
        return {}
    q = supabase.table("reviews").select("rm, checked, reviewed_by, updated_at").eq("periode_date", periode_date)
    if file_name:
        q = q.eq("file_name", file_name)
    data = q.execute().data or []
    return {row["rm"]: row for row in data}

def upsert_reviews(supabase, periode_date: str, file_name: str | None, rows_to_upsert):
    payload = []
    ...
    if payload:
        supabase.table("reviews").upsert(payload, on_conflict="periode_date,file_name,rm").execute()
...
def build_block_with_meta(no, row, visit_key, base_date):
    ...
    # flag konsultasi: kata 'Konsultasi' atau 'Konsul' di tindakan
...
st.caption("Porting dari app Tkinter: parsing PDF → tabel → template WA + shared memory (Supabase)")

with st.expander("Catatan & aturan format", expanded=False):
    st.markdown(
        "- **Nama multi-baris utuh**, NOPEN 8–18 digit, RM → `XX.XX.XX`.\n"
        "- **PERIODE** dipakai sebagai base tanggal kontrol & rekap harian.\n"
        "- Kunjungan 3: **Tindakan** satu baris (tanpa bullet).\n"
        "- Kontrol otomatis dari **POD** (dengan aturan Minggu → POD IV / +1 hari).\n"
        "- Status review disimpan bersama via Supabase (semua user lihat status sama).\n"
    )
...
if uploaded is not None:
    ...
    # Tanggal untuk header/rekap
    if period_date:
        per_date = period_date
    else:
        per_date = date.today()
    ...
    review_map = load_review_map(supabase, per_str_db, uploaded.name)
    if review_map:
    ...
    # Simpan perubahan status review (shared)
    ...
    for _, r in edited.iterrows():
        ...
        if new_checked != old_checked:
            changed.append({
                "rm": rm,
                "checked": new_checked,
                "reviewed_by": reviewer.strip() or None
            })

    if st.button("💾 Simpan status review (sync)", use_container_width=True, type="primary"):
        try:
            upsert_reviews(supabase, per_str_db, uploaded.name, changed)
            st.success(f"Status {len(changed)} pasien tersimpan & terbagi ke semua user.")
    ...
    header_lines = [
        ...
        f"Konsultasi\t      : {konsultasi_count} Pasien",  # pakai tab agar titik awal label tidak berubah
    ...
    with col2:
        ...
        st.write("- **Centang** pasien yang sudah direview, lalu klik **Simpan status review (sync)** agar tersimpan bersama.")
        st.write("- DPJP terisi otomatis via *fuzzy mapping* dari PDF.")

st.divider()
st.caption("Made for Streamlit Cloud — pdfplumber + python-docx + Supabase shared state")
